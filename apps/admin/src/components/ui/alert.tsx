  ...
  'relative grid w-full grid-cols-[0_1fr] items-start gap-y-0.5 rounded-lg border px-4 py-3 text-sm has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5',
  {
    variants: {
      variant: {
        ...
        warning:
          'border-warning/40 bg-warning/10 text-warning-foreground dark:text-warning [&>svg]:text-current',
        success:
          'border-success/30 bg-success/8 text-success [&>svg]:text-current *:data-[slot=alert-description]:text-success/90',
        info: 'border-info/30 bg-info/8 text-info [&>svg]:text-current *:data-[slot=alert-description]:text-info/90',
      },
    },
