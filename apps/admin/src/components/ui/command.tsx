  ...
  return (
    <Dialog {...props}>
      <DialogContent className={cn('overflow-hidden p-0', className)} showCloseButton={false}>
        {/*
          Inside DialogContent, not beside it: Radix only portals the content,
          so a header placed outside would render inline in the page and leak a
          stray absolutely-positioned element into the app shell — and the
          accessible name would not be associated with the dialog.
        */}
...
function CommandItem({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:bg-accent data-[selected=true]:text-accent-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none",
